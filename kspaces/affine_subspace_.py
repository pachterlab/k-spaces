...
class affine_subspace:
    ...
    def fit(self,points,responsibilities, objective, verbose):
        """If objective = L2 (ie total squared euclidean distance): fits subspace to points using  PCA/SVD if responsibilities are all 0 or 1, otherwise with np covariance.
        TLAD: minimizes total absolute euclidean distance from points to space with scipy optimize
        L1: minimizes L1 norm with scipy optimize. Pretty slow"""
        translation = None
        vectors = None
        latent_sigmas = None
        ...
        if self.d == 0 and len(points[mask]) == 1:
        ...
        elif alg == 'skcuda.linalg.PCA':
            pass
        elif alg == 'TLAD':
            #Minimize sum of absolute distances rather than the sum of absolute distances squared
            subspace_array = pack_subspace_array(self)
            def weighted_TLAD(subspace_array):
                return np.dot(responsibilities, orthogonal_distance(subspace_array,self.D,points))
            
            result = minimize(weighted_TLAD, subspace_array).x
            translation, vectors = unpack_subspace_array(result,self.D)
            print('using gaussian latent distribution (and also for E step). need to implement multivariate laplace distribution')
            
            ###########################
            ##### REPLACE LATER ####### NOT CORRECT, JUST A HACK TO SEE IF weighted_TLAD is working
            ###########################
            vectors = vectors_to_orthonormal_basis(vectors)
            latent_sigmas = []
            
            temp_space = affine_subspace(vectors, translation, -1, [-1], -1)
            projections = temp_space.projection(points - np.median(points, axis = 0))
            
            #weighted covariance matrix
            cov = np.cov(projections, rowvar = False, aweights = responsibilities)
            
            #eigenvalues and eigenvectors
            eigvals, eigvecs = np.linalg.eig(cov)
            #rare error likely due to numerical precision can produce complex eigenvalues close to 0
            ...
            #latent vector standard deviations are square roots of eiganvalues
            latent_sigmas = np.sqrt(eigvals[:self.d])
            ...
            #minimize L1 norm
            subspace_array = pack_subspace_array(self)
            def weighted_L1(subspace_array):
                return np.dot(responsibilities, L1_distance_subspace_array(subspace_array,self.D,points))
            result = minimize(weighted_L1, subspace_array).x
            translation, vectors = unpack_subspace_array(result,self.D)
            distances, selected_points = L1_distance_subspace_array(subspace_array,self.D,points, return_points = True)
            ## JUNK CODE ##
            latent_sigmas = np.ones(self.d)
            ###############
        else:
    ...
    def L1_distance_line(self, points, return_points = False):
        """
        For each point, the L1 distance from the point Q to the line needs to evaluated at each of the critical points of the L1 distance function along the line.
        These critical points correspond to the points on the line where an element in Q is matched (i.e. P = <1,2> Q = <0,2>)
        To parallelize this for both the N points and D critical points corresponding to each of the N points:
        1. line is an 1xD array. Duplicate it into a 3D NxDxD array, temp.
        2. Each of the D 2D arrays (applying our calculation along axis 2) will be used to evaluate one of the D critical points per each point Q_i.
            letting i be our index for axis 0 and j our index for axis 2:
            multiply temp[i,:,j] by points[i,j]/line[j] to obtain critical_points. This yields our ND critical points
        3. absolute value of points - critical points yields L1_distances_critical_points
        4. minimum L1_distances_critical_points along axis 1 yields our N L1 distances.
        (indices below are different because of how I used np tile)
        If an entry in the vector is 0, then the result of points[i,j]/line[j] is np.inf or np.nan (0/0 = nan).
            In the case of degenerate vectors (ie a null vector) all entries will be np.inf or np.nan, so multiplying by 1 ensures the np.min function works correctly.
        """
        vectors = self.vectors
        if len(vectors) > 1:
            raise RuntimeError('L1 distance not implemented for subspaces with d >1')
        elif len(vectors) == 0:
            vectors = [[0,0,0]]
            
        vector = vectors[0]
        points_ = points - self.translation
        critical_points = np.tile(vector, [len(points),self.D,1])
        #for j in range(self.D):
        #    critical_points[:,j,:] = (points_[:,j]/self.vector[j]).reshape(len(points),1)*critical_points[:,j,:]
        rescaling = points_ / vector #used chatGPT to eliminate for loop above
        rescaling = np.nan_to_num(rescaling, nan=1.0, posinf=1.0, neginf=1.0) # Replace np.nan and np.inf with 1.0
        critical_points = rescaling[:, :, np.newaxis] * critical_points #used chatGPT to eliminate for loop above
        critical_point_distances = np.sum(np.absolute(np.tile(points_,[1,1,self.D]).reshape(len(points_),self.D,self.D) - critical_points),axis = 2)
        L1_points_indices = np.argmin(critical_point_distances,axis = 1)
        selected_points = critical_points[range(len(critical_points)),L1_points_indices,:]
        L1_distances = critical_point_distances[range(len(critical_points)),L1_points_indices]
        if return_points:
...
def L1_distance_subspace_array(subspace_array,D,points, return_points = False):
    ...
    """
        ...
        If an entry in the vector is 0, then the result of points[i,j]/line[j] is np.inf or np.nan (0/0 = nan).
            In the case of degenerate vectors (ie a null vector) all entries will be np.inf or np.nan, so multiplying by 1 ensures the np.min function works correctly.
    """
    points_ = points - translation
    critical_points = np.tile(vector, [len(points),D,1])
    rescaling = points_ / vector 
    rescaling = np.nan_to_num(rescaling, nan=1.0, posinf=1.0, neginf=1.0) # Replace np.nan and np.inf with 1.0
    critical_points = rescaling[:, :, np.newaxis] * critical_points 
    critical_point_distances = np.sum(np.absolute(np.tile(points_,[1,1,D]).reshape(len(points_),D,D) - critical_points),axis = 2)
    L1_points_indices = np.argmin(critical_point_distances,axis = 1)
    selected_points = critical_points[range(len(critical_points)),L1_points_indices,:]
    L1_distances = critical_point_distances[range(len(critical_points)),L1_points_indices]
    if return_points:
