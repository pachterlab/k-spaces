...
def smart_init2(points,D=2, kd = [1,1], fixed_spaces = []):
    ...
    for i, d_i in enumerate(kd): #dimension for this subspace
        s = fit_single_space(points[kmeans.labels_ == i],d_i, 'L2', min_variance = 1e-10)
        s.prior = 1/k
        spaces.append(s)
...
def E_step(points, spaces,assignment = 'hard',verbose = False, norm = 'L2'):
    ...
    if assignment == 'closest':
        ...
        #distances = np.array([[space.orthogonal_distance(p) for space in spaces] for p in points])
        distances = []
        if norm == 'L1':
            distances = np.array([space.L1_distance_line(points) for space in spaces]).T
        else:
        ...
        if norm == 'L1':
            raise RuntimeError('L1 norm cannot be used with soft or hard assignment. Switch to "closest" assignment')
...
def M_step(spaces, points, probabilities, multiprocess_spaces, objective, verbose):
    ...
    objective: "L2" "L1" or "TLAD" (Total least absolute distance)
    verbose: bool
    ...
    if multiprocess_spaces:
        ...
        args = [(space,points,probabilities[:,i], objective, verbose) for i,space in enumerate(spaces)]
        ...
        spaces = [space.fit(points,probabilities[:,i], objective, verbose) for i,space in enumerate(spaces)]
        
    return spaces

def expectation_maximization_DA(points, spaces, EM_times, objective, max_iter=100, tol=5e-4, verbose = False, silent = False, 
    ...
    while stop == False:
        ...
        for _ in range(max_iter):
            ...
            spaces = M_step(spaces, points_, probabilities, multiprocess_spaces, objective, verbose)

            if set_noise_equal == True:
                set_sigmas_eq_noise(points, probabilities, spaces, objective, verbose)
        ...
        if beta>1:
            beta = 1
    probabilities = E_step(points, spaces, assignment = 'soft', norm = objective)

    if print_ownerships:
        print('final ownerships: ',[round(p,1) for p in np.sum(probabilities,axis = 0)])
    return spaces, probabilities, flag

def expectation_maximization(points, spaces, EM_times, objective, max_iter=100, tol=5e-4, verbose = False, silent = False, assignment = 'hard', 
                             print_ownerships = False, batch_size = np.inf, batch_replace = True, multiprocess_spaces = False, 
    ...
    for _ in range(max_iter):
        ...
        probabilities = E_step(points_, spaces, assignment = assignment, verbose= verbose, norm = objective)
        ...
        if check_convergence(prev,spaces, tolerance = tol) == True:
            probabilities = E_step(points, spaces, assignment = assignment, norm = objective) #note: points not points_
            if check_ownerships(probabilities, dims, num_fixed_spaces) == True: #check final M step
                print(f'Failed on iteration {_ + 1}: a space was eliminated')
                flag = 1
                break
    ...
    probabilities = E_step(points, spaces, assignment = assignment, norm = objective)
    
    if assignment == 'closest':
        set_sigmas_eq_noise(points, probabilities, spaces, objective, verbose) #see function description for explanation
        enforce_min_variance(spaces, min_variance, verbose)
    ...
    while i < initializations:
        ...
        ### STORE RESULT IF SUCCESSFUL ###
            ...
            if flag == 0:
                ...
                if objective == 'TLAD' or objective == 'L1':
                    print('no likelihood function for TLAD or L1 optimizations. Using loss function.')
                    model_likelihood = sum([np.dot(probabilities.T[i], s.L1_distance_line(points)) for i, s in enumerate(spaces)])
                    print(f'{objective} norm of model = {model_likelihood}')
                if objective == 'L2':
...
def fit_single_space(points,d, objective, min_variance = 1e-10):
    """ fits a single space with PCA
    points: N x D array
    d: int. dimension of space to fit
    objective: "L2", "L1", or "TLAD" (Total Least Absolute Distance). norm to be minimized
    ...
    space.fit(points,np.ones(len(points)), objective, False)
...
def fit_wrapper(space, points_, probabilities_, objective):
    """wrapper for using multiprocessing with affine_subspace.fit"""
    result =  space.fit(points_,probabilities_, objective)
    return space, result

def set_sigmas_eq_noise(points, probabilities, spaces, objective, verbose):
    ...
    distances = None
    if objective == 'L1':
        print('computing total sum of squared distances for a variance to use in model likelihood. Not sure if this makes sense for computing the variance if distances are L1.')
        distances = np.array([space.L1_distance_line(points) for space in spaces_]).T
    
    elif objective == 'L2' or objective == 'TLAD':
